...
const addLiquidity = (account, { token0, token1, manager }) => {
  ...
  const amount0 = ethers.utils.parseEther("0.78818986061");
  const amount1 = ethers.utils.parseEther("1999.94041212"); // 5000 USDC
  const lowerTick = 73135;
  const upperTick = 78244;
  const liquidity = ethers.BigNumber.from("333804772048279699456");
...
const SwapForm = (props) => {
  ...
  const swap_ = (e) => {
    e.preventDefault();
    swap(zeroForOne, zeroForOne ? amount0 : amount1, metamaskContext.account, {
      tokenIn: token0,
      manager,
      token0,
      token1,
  ...
  if (token0 && token1) {
    token0
      ...
      .then((res) => console.log("POOL", res.toString()));
  ...
  return (
    <section className="SwapContainer">
      <header>
        <h1>Swap tokens</h1>
        <button disabled={!enabled || loading} onClick={addLiquidity_}>
          Add liquidity
        </button>
      </header>
      <form className="SwapForm">
        ...
        />
        <button className="swap" disabled={!enabled || loading} onClick={swap_}>
          Swap
        </button>
      </form>
    </section>
